...
const state = {
  ...
  groups: [],                  // my groups (fetched on dashboard load)
  groupChannel: null           // realtime channel for the open group chat
};

let currentGroup = null;       // group object of the open group chat
...
function navigateTo(viewName) {
  const views = {
    ...
    'group-chat': document.getElementById('group-chat-view')
  };

  // Leaving the group chat → tear down its realtime channel + clear typing
  if (viewName !== 'group-chat' && currentGroup) {
    setGroupTyping(currentGroup.id, false).catch(() => {});
    if (state.groupChannel) {
      state.groupChannel.unsubscribe();
...
function handleDeepLinks() {
  ...
  // Group notification tap: /?group=<groupId> → open that group chat
    ...
    setTimeout(async () => {
      try {
        ...
        if (group) {
          ...
          showToast('You are not a member of that group.', 'info');
        }
      } catch { /* ignore */ }
    }, 400);
    const clean = new URLSearchParams(_savedSearch);
...
/* ==========================================
   GROUPS — render, create, and group chat
   ========================================== */
async function loadGroups() {
  try {
    state.groups = await fetchMyGroups();
  } catch (err) {
    console.warn('[Pulse] Groups fetch failed:', err.message);
    state.groups = [];
  }
  renderGroups();
}
...
function renderGroups() {
  ...
  if (groups.length === 0) {
    container.innerHTML = `
      <div style="font-size: 13px; color: hsl(var(--text-muted)); font-style: italic; padding: 4px 0;">
        No groups yet — create one to chat with your friends together!
      </div>`;
    return;
  }

  container.innerHTML = groups.map(g => {
    const memberNames = (g.members || []).map(m => m.name || m.username || 'Friend').slice(0, 3);
    const more = (g.members || []).length > 3 ? ` +${(g.members || []).length - 3}` : '';
    return `
      <div class="group-item" data-gid="${escapeHtml(g.id)}" role="button" tabindex="0">
        <div class="group-avatar">👥</div>
        <div class="group-info">
          <span class="group-name">${escapeHtml(g.name)}</span>
          <span class="group-members">${escapeHtml(memberNames.join(', ') + more)}</span>
        </div>
        <svg class="icon group-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
      </div>`;
  }).join('');

  container.querySelectorAll('.group-item').forEach(el => {
    const open = () => openGroupChat(el.dataset.gid);
...
function updateGroupChatHeader() {
  ...
  const count = (currentGroup.members || []).length;
  if (metaEl) metaEl.textContent = count > 0 ? `${count} members` : '';
}
...
async function openGroupManageModal() {
  ...
  const subtitle = document.getElementById('group-manage-subtitle');
  const list = document.getElementById('group-manage-members');
  const myId = state.userProfile?.id;
  const isCreator = currentGroup.created_by === myId;
  const isAdmin = isCreator || (currentGroup.members || []).find(m => m.id === myId)?.role === 'admin';

  if (title) title.textContent = currentGroup.name;
  if (subtitle) subtitle.textContent = isCreator ? 'You are the group creator.' : (isAdmin ? 'You are an admin.' : 'Group info');

  const deleteBtn = document.getElementById('group-manage-delete');
  const addBtn = document.getElementById('group-manage-add');
  const renameToggle = document.getElementById('group-manage-rename-toggle');
  ...
  if (renameToggle) renameToggle.style.display = isAdmin ? 'flex' : 'none';
  if (renameInput) renameInput.value = currentGroup.name;
  ...
  try {
    const members = await fetchGroupMembers(currentGroup.id);
    currentGroup.members = members;
    updateGroupChatHeader();
    list.innerHTML = members.length
      ? members.map(m => {
          const isMe = m.id === myId;
          const isOwner = m.id === currentGroup.created_by;
          const role = m.role || (isOwner ? 'creator' : 'member');
          const roleBadge = isOwner
            ? '<span class="group-role-badge creator">Creator</span>'
            : role === 'admin'
              ? '<span class="group-role-badge admin">Admin</span>'
              : '';
          // Creator can promote/demote; admins can remove members
          const roleCtl = isCreator && !isOwner && !isMe
            ? (role === 'admin'
                ? `<button class="btn btn-secondary btn-small group-manage-role" data-uid="${escapeHtml(m.id)}" data-role="member" data-name="${escapeHtml(m.name || 'Friend')}" style="padding:3px 8px;font-size:10px;">Demote</button>`
                : `<button class="btn btn-secondary btn-small group-manage-role" data-uid="${escapeHtml(m.id)}" data-role="admin" data-name="${escapeHtml(m.name || 'Friend')}" style="padding:3px 8px;font-size:10px;">Make admin</button>`)
            : '';
          const removeCtl = isAdmin && !isOwner && !isMe
            ? `<button class="btn btn-secondary btn-small group-manage-remove" data-uid="${escapeHtml(m.id)}" data-name="${escapeHtml(m.name || 'Friend')}" style="padding:4px 10px;font-size:11px;">Remove</button>`
            : '';
          return `
            <div class="group-manage-row">
              <span class="group-member-avatar">👤</span>
              <span class="group-manage-name">${escapeHtml(m.name || 'Friend')}${isMe ? ' <span class="tag">You</span>' : ''}</span>
              ${roleBadge}
              <span class="group-manage-username">${m.username ? `@${escapeHtml(m.username)}` : ''}</span>
              ${roleCtl}
              ${removeCtl}
            </div>`;
        }).join('')
      : '<div style="font-size:13px;color:hsl(var(--text-muted));font-style:italic;text-align:center;">No members.</div>';

    // Role change (creator only)
    list.querySelectorAll('.group-manage-role').forEach(btn => {
      btn.addEventListener('click', async () => {
        try {
          await setGroupMemberRole(currentGroup.id, btn.dataset.uid, btn.dataset.role);
          showToast(btn.dataset.role === 'admin' ? `${btn.dataset.name} is now an admin.` : `${btn.dataset.name} is no longer an admin.`);
    ...
    list.querySelectorAll('.group-manage-remove').forEach(btn => {
      btn.addEventListener('click', async () => {
        const uid = btn.dataset.uid;
        const ok = await showConfirmModal({
          icon: '🚫',
          title: 'Remove member?',
          body: `Remove ${btn.dataset.name} from "${currentGroup.name}"?`,
          okLabel: 'Remove'
        });
        if (!ok) return;
        try {
          await removeGroupMember(currentGroup.id, uid);
          showToast(`${btn.dataset.name} removed.`);
          await loadGroups();
          currentGroup = (state.groups || []).find(g => g.id === currentGroup.id) || currentGroup;
          openGroupManageModal();
        } catch (err) {
    ...
    console.warn('[Pulse] Manage modal members fetch failed:', err.message);
    list.innerHTML = '<div style="font-size:13px;color:hsl(var(--text-muted));font-style:italic;text-align:center;">Could not load members.</div>';
  }
}

// Highlight @username mentions in group messages
function renderGroupMentions(text, members = []) {
  if (!text) return '';
  const usernames = new Set((members || []).map(m => m.username).filter(Boolean));
  const escaped = escapeHtml(text);
  return escaped.replace(/@([a-z0-9_]{1,32})/gi, (match, name) => {
    const lower = name.toLowerCase();
    if (usernames.has(lower)) {
      return `<span class="group-mention-chip">@${escapeHtml(name)}</span>`;
    }
    return match;
  });
}

// "Seen by N" — count members whose last_read_at >= this message's created_at
function renderGroupSeenBy(msg) {
  const myId = state.userProfile?.id;
  if (msg.sender_id !== myId) return '';
  const created = new Date(msg.created_at).getTime();
  const readers = (currentGroup?.members || [])
    .filter(m => m.id !== myId && m.last_read_at && new Date(m.last_read_at).getTime() >= created);
  if (readers.length === 0) return '';
  return `<span class="group-seen-by" title="${readers.map(r => r.name || 'Friend').join(', ')}">Seen by ${readers.length}</span>`;
}

function refreshGroupReadReceipts() {
  const container = document.getElementById('group-chat-messages');
  if (!container) return;
  container.querySelectorAll('.chat-bubble.sent').forEach(bubble => {
    const id = bubble.dataset.gmsgId;
    const seenEl = bubble.querySelector('.group-seen-by');
    const msg = { id, sender_id: state.userProfile?.id, created_at: bubble.dataset.created };
    const html = renderGroupSeenBy(msg);
    const meta = bubble.querySelector('.chat-meta');
    if (html) {
      if (seenEl) seenEl.remove();
...
function renderGroupMessageHtml(msg) {
  const myId = state.userProfile?.id;
  const isSent = msg.sender_id === myId;
  const time = new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  const senderName = msg.sender?.name || msg.sender?.username || 'Friend';
  ...
  const senderHtml = isSent
    ? ''
    : `<div class="group-msg-sender">${escapeHtml(senderName)}</div>`;

  return `
    <div class="chat-row ${isSent ? 'sent' : 'received'}">
      <div class="chat-col">
        <div class="chat-bubble ${isSent ? 'sent' : 'received'}" data-gmsg-id="${escapeHtml(msg.id)}"
          data-created="${escapeHtml(msg.created_at)}">
          ${senderHtml}
          ${msg.content_text ? `<div>${renderGroupMentions(msg.content_text, currentGroup?.members)}</div>` : ''}
          ${mediaHtml}
          <div class="chat-meta">
            <span class="chat-bubble-time">${time}</span>
            ${renderGroupSeenBy(msg)}
          </div>
        </div>
      </div>
    </div>
...
function appendGroupMessage(msg) {
  const container = document.getElementById('group-chat-messages');
  if (!container) return;
  if (container.querySelector('[data-gmsg-id]')) {
    container.insertAdjacentHTML('beforeend', renderGroupMessageHtml(msg));
  } else {
    container.innerHTML = renderGroupMessageHtml(msg);
  }
  container.scrollTop = container.scrollHeight;
}
...
async function openGroupChat(groupId) {
  ...
  const container = document.getElementById('group-chat-messages');
  container.innerHTML = '<div class="spinner" style="margin:auto;"></div>';
  const typingEl = document.getElementById('group-typing-indicator');
  if (typingEl) typingEl.style.display = 'none';

  try {
    const messages = await fetchGroupMessages(groupId);
    container.innerHTML = messages.length
      ? messages.map(renderGroupMessageHtml).join('')
      : '<div style="text-align:center;color:hsl(var(--text-muted));margin-top:40px;">No messages yet — say hi! 👋</div>';
    container.scrollTop = container.scrollHeight;
    // Mark everything as read by me (watermark on my membership row)
    markGroupRead(groupId).catch(() => {});
  } catch (err) {
    console.warn('[Pulse] Group messages fetch failed:', err.message);
    container.innerHTML = '<div style="text-align:center;color:hsl(var(--text-muted));margin-top:40px;">Could not load messages.</div>';
  }

  updateGroupChatHeader();

  // Realtime — messages, membership, typing + read receipts
  if (state.groupChannel) state.groupChannel.unsubscribe();
  state.groupChannel = subscribeToGroupMessages(groupId, async (evt) => {
    if (!currentGroup || currentGroup.id !== groupId) return;
    if (evt.type === 'members_changed') {
      // Member added/removed by the creator — refresh count + member list
      try {
        const fresh = await fetchGroupMembers(groupId);
        currentGroup.members = fresh;
        updateGroupChatHeader();
        // Keep the manage modal in sync if it's open
        const manageModal = document.getElementById('group-manage-modal');
        if (manageModal && manageModal.style.display === 'flex') openGroupManageModal();
      } catch { /* ignore */ }
      return;
    }
    if (evt.type === 'reads_changed') {
      // Someone read the chat — refresh "seen by N" on my sent messages
      try {
        const fresh = await fetchGroupMembers(groupId);
        currentGroup.members = fresh;
        refreshGroupReadReceipts();
      } catch { /* ignore */ }
      return;
    }
    if (evt.type === 'typing') {
      const myId = state.userProfile?.id;
      if (!evt.user_id || evt.user_id === myId) return;
      const typer = (currentGroup.members || []).find(m => m.id === evt.user_id);
      if (typingEl) {
        if (evt.typing) {
          typingEl.textContent = `${typer?.name || 'Someone'} is typing…`;
    ...
    // Attach sender profile for the name label
    try {
      const { data } = await client()
        ...
        .eq('id', msg.sender_id)
        .single();
      msg.sender = data || { name: 'Friend' };
    } catch { /* keep anonymous */ }
    appendGroupMessage(msg);
    // I'm looking at the chat — bump my read watermark so senders see "seen by"
    markGroupRead(groupId).catch(() => {});
  });
}

async function sendGroupMessageNow() {
  ...
  input.value = '';
  try {
    const msg = await sendGroupMessage(currentGroup.id, text);
    appendGroupMessage(msg);
    // Push notification to other members (fires when their app is closed)
    notifyGroupOfMessage(
      currentGroup.id,
      state.userProfile?.name || 'Someone',
      text,
      null,
      currentGroup.name
    );
...
function initEventListeners() {
  ...
  // Create Group modal — open, select-all toggle, cancel, create
  const renderGroupMemberPicker = (friends, containerId) => {
    const picker = document.getElementById(containerId);
    if (!picker) return;
    if (!friends.length) {
      picker.innerHTML = `<div style="font-size:13px;color:hsl(var(--text-muted));font-style:italic;">
        No connected friends available.</div>`;
      return;
    }
    picker.innerHTML = friends.map(f => `
      <label class="group-member-option">
        <input type="checkbox" value="${escapeHtml(f.friendId)}" class="group-member-check">
        <span class="group-member-avatar">${escapeHtml(f.statusEmoji || '😊')}</span>
        <span class="group-member-name">${escapeHtml(f.displayName || f.name)}</span>
        ${f.username ? `<span class="group-member-username">@${escapeHtml(f.username)}</span>` : ''}
      </label>`).join('');
  };

  const syncGroupPickerCount = () => {
    const checked = document.querySelectorAll('#group-member-picker .group-member-check:checked').length;
    const counter = document.getElementById('group-picker-count');
  ...
  document.getElementById('btn-create-group')?.addEventListener('click', () => {
    ...
    renderGroupMemberPicker(friends, 'group-member-picker');
    syncGroupPickerCount();
    document.querySelectorAll('#group-member-picker .group-member-check').forEach(cb => {
      cb.addEventListener('change', syncGroupPickerCount);
    });
    const saBtn = document.getElementById('group-picker-select-all');
    if (saBtn) {
      saBtn.textContent = 'Select all';
      saBtn.onclick = () => {
        const cbs = document.querySelectorAll('#group-member-picker .group-member-check');
        const allOn = [...cbs].every(cb => cb.checked);
        cbs.forEach(cb => { cb.checked = !allOn; });
        saBtn.textContent = allOn ? 'Select all' : 'Clear';
        syncGroupPickerCount();
      };
    }
  ...
  document.getElementById('group-create-save')?.addEventListener('click', async () => {
    const name = document.getElementById('group-name-input')?.value.trim() || '';
    const checks = [...document.querySelectorAll('.group-member-check:checked')].map(cb => cb.value);
    if (!name) {
      showToast('Give your group a name.', 'error');
      return;
    }
    if (checks.length === 0) {
      showToast('Pick at least one friend to add.', 'error');
      return;
    }
    const btn = document.getElementById('group-create-save');
    btn.disabled = true;
    try {
      await createGroup(name, checks);
      document.getElementById('group-create-modal').style.display = 'none';
  ...
  // Group chat — back, send (button + Enter), header info button
  document.getElementById('group-chat-back-btn')?.addEventListener('click', () => {
    navigateTo('dashboard');
  });
  document.getElementById('group-chat-send')?.addEventListener('click', sendGroupMessageNow);
  ...
  document.getElementById('group-manage-close')?.addEventListener('click', closeGroupManage);

  document.getElementById('group-manage-add')?.addEventListener('click', () => {
    if (!currentGroup) return;
    const memberIds = new Set((currentGroup.members || []).map(m => m.id));
    const friends = (state.connections || [])
      .filter(c => c.status === 'connected')
      .filter(c => !memberIds.has(c.friendId));
    document.getElementById('group-add-name').textContent = currentGroup.name;
    renderGroupMemberPicker(friends, 'group-add-picker');
    document.getElementById('group-add-members-modal').style.display = 'flex';
  });

  document.getElementById('group-add-cancel')?.addEventListener('click', () => {
    document.getElementById('group-add-members-modal').style.display = 'none';
  });

  document.getElementById('group-add-save')?.addEventListener('click', async () => {
    if (!currentGroup) return;
    const checks = [...document.querySelectorAll('#group-add-picker .group-member-check:checked')].map(cb => cb.value);
    if (!checks.length) {
      showToast('Pick at least one friend.', 'error');
      return;
    }
    const btn = document.getElementById('group-add-save');
    btn.disabled = true;
    try {
      await addGroupMembers(currentGroup.id, checks);
      document.getElementById('group-add-members-modal').style.display = 'none';
  ...
  document.getElementById('group-manage-leave')?.addEventListener('click', async () => {
    if (!currentGroup) return;
    const ok = await showConfirmModal({
      icon: '🚪',
      title: 'Leave group?',
      body: `You'll no longer see messages in "${currentGroup.name}". You can only rejoin if a member adds you back.`,
      okLabel: 'Leave'
    });
    if (!ok) return;
    try {
  ...
  document.getElementById('group-manage-delete')?.addEventListener('click', async () => {
    if (!currentGroup) return;
    const ok = await showConfirmModal({
      icon: '🗑️',
      title: 'Delete group?',
      body: `"${currentGroup.name}" and all its messages will be permanently deleted. This cannot be undone.`,
      okLabel: 'Delete'
    });
    if (!ok) return;
    try {
  ...
  // Rename group (toggle row, save)
  document.getElementById('group-manage-rename-toggle')?.addEventListener('click', () => {
    const row = document.getElementById('group-rename-row');
    const input = document.getElementById('group-rename-input');
    if (!row || !currentGroup) return;
  ...
  document.getElementById('group-rename-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('group-rename-row').style.display = 'none';
  ...
  // Group chat — typing indicator (broadcast on input, throttle)
  let _groupTypingTimer = null;
  const groupInput = document.getElementById('group-chat-input');
  if (groupInput) {
    groupInput.addEventListener('input', () => {
      if (!currentGroup) return;
      setGroupTyping(currentGroup.id, true).catch(() => {});
      clearTimeout(_groupTypingTimer);
      _groupTypingTimer = setTimeout(() => {
        setGroupTyping(currentGroup.id, false).catch(() => {});
      }, 1200);
    });
    groupInput.addEventListener('blur', () => {
      if (currentGroup) setGroupTyping(currentGroup.id, false).catch(() => {});
    });
  }
  // Stop typing broadcast when leaving the group chat
  const origGroupBack = document.getElementById('group-chat-back-btn');
  if (origGroupBack) {
    const _orig = origGroupBack.onclick;
    origGroupBack.addEventListener('click', () => {
      if (currentGroup) setGroupTyping(currentGroup.id, false).catch(() => {});
    });
  }

  // Group chat — @mention picker
  const mentionPicker = document.getElementById('group-mention-picker');
  if (groupInput && mentionPicker) {
    ...
    const showMentionPicker = (members, caret) => {
      mentionMembers = members;
      mentionIndex = 0;
      mentionPicker.innerHTML = members.map((m, i) =>
        `<div class="group-mention-option${i === 0 ? ' active' : ''}" data-midx="${i}">
          <span class="group-member-avatar">👤</span>
          <span class="group-mention-name">${escapeHtml(m.name || 'Friend')}</span>
          ${m.username ? `<span class="group-member-username">@${escapeHtml(m.username)}</span>` : ''}
        </div>`
      ).join('');
      mentionPicker.style.display = 'block';
      mentionPicker.dataset.caret = String(caret);
      mentionPicker.querySelectorAll('.group-mention-option').forEach(el => {
        el.addEventListener('click', () => insertMention(Number(el.dataset.midx)));
      });
    };
    const insertMention = (idx) => {
      const m = mentionMembers[idx];
      if (!m) return;
      const caret = Number(mentionPicker.dataset.caret || groupInput.selectionStart || 0);
      const val = groupInput.value;
      // Find the @token start before the caret
      const atIdx = val.lastIndexOf('@', caret - 1);
      const start = atIdx >= 0 ? atIdx : caret;
      const handle = m.username ? `@${m.username}` : `@${(m.name || 'friend').replace(/\s+/g, '_').toLowerCase()}`;
      groupInput.value = val.slice(0, start) + handle + ' ' + val.slice(caret);
      groupInput.focus();
    ...
    groupInput.addEventListener('input', () => {
      const caret = groupInput.selectionStart || 0;
      const before = groupInput.value.slice(0, caret);
      const atIdx = before.lastIndexOf('@');
      if (atIdx === -1) {
        closeMentionPicker();
        return;
      }
      const token = before.slice(atIdx + 1).toLowerCase();
      if (!/^[a-z0-9_]{0,24}$/.test(token)) {
        closeMentionPicker();
        return;
      }
      ...
      showMentionPicker(members.slice(0, 6), caret);
    });
    groupInput.addEventListener('keydown', (e) => {
      if (mentionPicker.style.display !== 'block') return;
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        mentionIndex = (mentionIndex + 1) % mentionMembers.length;
        mentionPicker.querySelectorAll('.group-mention-option').forEach((el, i) => {
          el.classList.toggle('active', i === mentionIndex);
        });
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        mentionIndex = (mentionIndex - 1 + mentionMembers.length) % mentionMembers.length;
        mentionPicker.querySelectorAll('.group-mention-option').forEach((el, i) => {
          el.classList.toggle('active', i === mentionIndex);
        });
  ...
  document.getElementById('btn-deactivate-account')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (state.groupChannel) { state.groupChannel.unsubscribe(); state.groupChannel = null; }
      currentGroup = null;
      navigateTo('auth');
      setAuthMode('signin');
    } catch (err) {
  ...
  document.getElementById('btn-delete-account')?.addEventListener('click', async () => {
    const ok = await showConfirmModal({
      ...
      body: 'Your profile, connections, messages, groups and uploaded media will be permanently deleted after a 30-day grace period. You can cancel anytime before then. This cannot be undone.',
      okLabel: 'Delete'
    });
    if (!ok) return;
    try {
...
function setupAndroidBackButton() {
  ...
  import('@capacitor/app').then(({ App }) => {
    App.addListener('backButton', () => {
      ...
      // 3b) Close group modals (create / manage / add-members)
      const gcm = document.getElementById('group-create-modal');
      if (gcm && gcm.style.display === 'flex') {
        gcm.style.display = 'none';
        return;
      }
      const gmm = document.getElementById('group-manage-modal');
      if (gmm && gmm.style.display === 'flex') {
        gmm.style.display = 'none';
        return;
      }
      const gamm = document.getElementById('group-add-members-modal');
      if (gamm && gamm.style.display === 'flex') {
        gamm.style.display = 'none';
        return;
      }
      ...
      // 5b) Inside a group chat → back to the dashboard
      if (currentGroup) {
        navigateTo('dashboard');
        return;
      }
