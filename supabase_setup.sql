...
as $$
begin
  -- Atomic: claim an existing slot only when its timestamp is stale.
  -- Row-level locking serializes concurrent calls, so a second caller sees
  -- the freshly-updated timestamp and is correctly throttled.
  ...
  -- No row yet → insert (first caller wins; concurrent insert loses).
